...
- **HP 자연 감소**: `PirateLord`가 매 프레임 `ShipBody.OnDamaged(decayPerSec * dt)` 호출. **BossData.InvincibleTime=0**으로 두면 i-frame 가드가 비활성이라 매 프레임 차감이 그대로 적용됨 (ShipBody 표면 변경 없음)
...
- **추격 속도**: P1/P2보다 빠름 (`BossData.ChaseSpeed[2]`)
...
- `BossData.cs` — 보스 통합 데이터 (HP, 페이즈 임계, 페이즈별 추격 속도, P3 decay rate, 각 패턴 Config 참조)
...
| 페이즈별 추격 속도 (3개) | `BossData.ChaseSpeed[]` | 플레이어 최대 속도 기반 |
| P1→P2 HP 임계 | `BossData.Phase1ToPhase2Hp` | 초안 50% |
| P3 HP decay rate | `BossData.Phase3DecayPerSec` | "도망 승리" 체감 시간 결정 |
